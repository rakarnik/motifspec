...
comp_motif(m) 
{
  ...
  comp_fmsize = (m.get_width() + 2 * m.ncols()) * 4;
  ...
  for(i = comp_r1 + 4; i < comp_fmsize; i += 4){
    i1 = 2.0;
    for(j = 0; j < 4; j++){
      if(comp_fm1[i + j] > 0.0) i1 += comp_fm1[i + j] * log(comp_fm1[i + j])/lg2;
    }
  ...
  comp_r2 = comp_fmsize - 4 - comp_l1;
  comp_l2 = comp_fmsize - 4 - comp_r1;
...
double CompareACESites::compare(const CompareACESites &c){
  ...
  ref1 = max(ref1, comp_r1 - c.comp_fmsize + 4 + c.comp_l1);
  ref2 = min(comp_fmsize - 4 - c.comp_r1 + c.comp_l1,comp_r1);
  ref2 = min(ref2, c.comp_l1 + comp_l1);
  ref1rc = max(0, comp_l2 - c.comp_r1 + c.comp_l1);
  ref1rc = max(ref1rc,comp_r2 - c.comp_fmsize + 4 + c.comp_l1);
  ref2rc = min(comp_fmsize - 4 - c.comp_r1 + c.comp_l1, comp_r2);
  ...
  for(ref = ref1; ref <= ref2; ref += 4) {
    x1 = c.comp_l1;
		y1 = c.comp_r1;
		x2 = ref;
		y2 = ref + c.comp_r1 - c.comp_l1;
    if(x2 > comp_l1){
      x1 -= (x2 - comp_l1);
      x2 -= (x2 - comp_l1);
    }
  ...
  for(ref = ref1rc; ref <= ref2rc; ref += 4) {
    x1 = c.comp_l1;
		y1 = c.comp_r1;
		x2 = ref;
		y2 = ref + c.comp_r1 - c.comp_l1;
